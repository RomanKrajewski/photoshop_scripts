...
(function(){
    ...
    function convertLayerToData(layer){
        ...
        data.isExportable = data.startedVisible && !!data.filename ;
    ...
    var getPath = function(){
        ...
        var path = '~/PhotoshopExports/' + documentName.substring(0,documentName.lastIndexOf('.'));
    ...
    var exportLayer = function(data){
        ...
        var save = function(filename){
            // if(data.startedVisible){                 doesnt work for children because the script only checks the visibility of the parent layer and exports all combinations 
            //Save
            //prepare path
            if(filename.lastIndexOf('/')!=-1){
                var newFolder = new Folder(getPath() + filename.substring(0, filename.lastIndexOf('/')));
                if(!newFolder.exists){
                    newFolder.create();
                }
            }
        ...
        if(new RegExp(/\{[^}]+\}/).test(data.filename)){
            ...
            var allTags = {};
            data.descendents.each(function(d){
                if(!d.startedVisible){//exclude layers without tags and descendends that were not visible at the start
...
var layerCompArray = doc.layerComps;
var outputFolder = Folder.selectDialog("Select an output folder");

var useLayerComps = confirm("use layer comps?")
if(!useLayerComps){
    exportLayersOfComp();

}
else{
for(var layerCompIndex = 0; layerCompIndex < layerCompArray.length;  layerCompIndex++){

            var answer = confirm("current layer comp: " + layerCompIndex  + ". continue?");
            if(!answer){
                return;
            }

            layerCompArray[layerCompIndex].visibility = true;
            layerCompArray[layerCompIndex].appearence = true;
            layerCompArray[layerCompIndex].position = true;

            layerCompArray[layerCompIndex].apply();
            exportLayersOfComp();
}
}

            
function exportLayersOfComp(){
